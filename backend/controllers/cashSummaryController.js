// controllers/cashSummaryController.js
...
async function sumRangeByDay(type, startDate, endDate, warehouseId) {
  let total = 0;
  const day = new Date(startDate); day.setHours(0,0,0,0);
...
// ─── sum each day’s latest VAN_CASH in [start→end] ────────────────────────
...
// ─── compute “liveBalance” for today (unchanged) ───────────────────────────
async function getCurrentLiveBalance(warehouseId) {
  ...
  const vanCashEntry = await Ledger.findOne({
  ...
  const vanCash = await sumDailyVanCash(today, todayEnd, warehouseId);
  const beforeX = opening + deposit + cash;
  const afterX  = beforeX - transfer;
  const diff    = vanCash ? vanCash - beforeX : 0;
...
exports.getCashSummary = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      // 1) build local-midnight range
      let startDate, endDate;
      if (qStart && qEnd) {
        startDate = parseLocalDay(qStart);
        endDate   = parseLocalDay(qEnd);
        endDate.setHours(23,59,59,999);
      } else {
        const pick = date || new Date().toISOString().slice(0,10);
        startDate = parseLocalDay(pick);
      ...
      const diffLastDay     = vanCashLastDay ? vanCashLastDay - beforeTransfers : 0;
      const closingBalance  = vanCashLastDay
