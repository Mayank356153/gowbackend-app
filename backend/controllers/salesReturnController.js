...
exports.createReturn = async (req, res) => {
  try {
    const {
      originalSaleRef,
      returnCode, referenceNo, returnDate,
      warehouse, customer, items,
      totalRefund, status, note,
      otherCharges, discountCouponCode,
      couponType, couponValue, discountOnAll,
      payments
    } = req.body;

    if (!originalSaleRef || !returnCode || !warehouse || !customer || !items?.length) {
    ...
    // **1) update inventory ledger (add back)**
