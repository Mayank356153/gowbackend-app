...
const ProfileEdit = () => {
  ...
  const fetchUser = async () => {
    ...
    try {
      const response = await axios.get(
          `https://mybackend-l7om.onrender.com/admiaddinguser/userlist`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(`https://mybackend-l7om.onrender.com/admiaddinguser/${editId}`, payload, { headers });
        alert("User updated");
      } else {
        await axios.post("https://mybackend-l7om.onrender.com/admiaddinguser/adduserbyadmin", payload, { headers });
        alert("User created");
      }
