...
function ClickHandler({ setStartPoint, setEndPoint, startPoint, circleRadius }) {
  useMapEvents({
    click(e) {
      ...
      if (!startPoint) {
        // कोई startPoint नहीं है → यह नया startPoint बने
        setStartPoint(clickedPoint);
      } else if (startPoint) {
        // अगर पहले से startPoint है → check करो कि click circle के बाहर है या नहीं
        ...
        if (dist > circleRadius) {
          // अगर circle के बाहर है → नया startPoint बना दो
          setStartPoint(clickedPoint);
          setEndPoint(null); // reset end point
        } else {
          // अगर circle के अंदर है → यह End Point बनेगा
...
const fetchRouteORS = async (points, setPath) => {
  try {
    ...
    const response = await axios.get("http://localhost:5000/api/way/many", {
...
export default function SaleLocation() {
  ...
  const fetchBills = async () => {
    ...
    console.log(data.filter(b=>b.warehouse._id="689c3e3b4ebe358ebc737346"));
    setBills(
      data.filter(
