...
export default function AddSalesReturn() {
  ...
  // Push item
  const pushItem = it => {
    if (items.some(i => i.item === it._id)) {
      alert("This item is already added.");
      return;
    }
    const saleItem = originalSale
      ? (originalSale.items || []).find(oi =>
          (oi.item?._id === it._id) || (oi.variant === it._id) || (oi.product === it._id)
        )
      : null;
    setItems(prev => [
      ...prev,
      {
        ...
        quantity: 1,
        reason: "",
        price: saleItem ? (saleItem.price || it.salesPrice || 0) : (it.salesPrice || 0),
        maxQuantity: saleItem ? (saleItem.quantity || 1) : 9999, // Arbitrary high limit if no sale
        isVariant: !!(it.parentId || it.variantId),
      },
    ]);
    setSearchTerm("");
    setSuggestions([]);
        ...
        <div className="flex-grow p-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2">
            <div className="relative flex flex-col">
              ...
              {saleSuggestions.length > 0 && (
                <ul className="absolute z-50 w-full mt-1 overflow-y-auto bg-white border rounded shadow max-h-60 top-full">
              ...
              {saleError && (
                <p className="mt-1 text-sm text-red-500">{saleError}</p>
              )}
            </div>
          ...
          <div className="mb-6 overflow-x-auto">
            <table className="w-full border table-auto">
              <thead className="text-white bg-sky-600">
                <tr>
                ...
                {items.length === 0 ? (
                  ...
                  items.map((it, idx) => (
                    <tr key={idx} className="text-center">
                      ...
                      <td className="px-2 py-1 border">{it.price.toFixed(2)}</td>
                      <td className="px-2 py-1 border">
                        <input
                          type="text"
          ...
          <div className="flex justify-center space-x-4">
            ...
            <button
              className="px-6 py-2 text-white bg-gray-600 rounded"
