...
const [page, setPage] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(
          "https://mybackend-l7om.onrender.com/api/subcategories",
        ...
        const masterResponse = await axios.get(
          'http://localhost:5000/api/items/getImageBySubCategory',
          { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
        );
  ...
  const handleRegenerateMasterImage = async (id) => {
    ...
    const res = await axios.get(
      `http://localhost:5000/api/items/subcategory/assign-masterImage/?page=${page}`,{
        categoryId:id
      },
      {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
   ...
   const handleSubmit=async(image)=>{
      const tokenHeader =  () => ({
        ...
        try{
            ...
            "http://localhost:5000/api/subcategories/assign/master-image",{
