   ...
   const[category,setCategory]=useState({})
    ...
    const[selectedItem,setSelectedItem]=useState({})
     ...
     const fetchItems = async () => {
        try {
            const response = await axios.get('http://localhost:5000/api/items/getWithsubsubCategory',{
                headers: {
...
setCategory(response.data.data)
     ...
     const fetchCats = async () => {
        try {
            console.log("asd")
            const response = await axios.get('http://localhost:5000/api/items/getImageBySubSubCategory',{
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                  }});
          ...
          }
 {
  ...
  <table className='min-w-full border-2 divide-y divide-gray-200 min-h-48'>
    <thead className='sticky top-0 bg-gray-200'>
      <tr>
        <th className='py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase'>SubSubCategory Id</th>
