...
const [page, setPage] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(
          "https://mybackend-l7om.onrender.com/api/sub-subcategories",
        ...
        const masterResponse = await axios.get(
          'http://localhost:5000/api/items/getImageBySubSubCategory',
          { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
        );
  ...
  const handleRegenerateMasterImage = async (id) => {
    ...
    const res = await axios.get(
      `http://localhost:5000/api/items/subsubcategory/assign-masterImage/?page=${page}`,{
        categoryId:id
      },
      {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
   ...
   const handleSubmit=async(image)=>{
      const tokenHeader =  () => ({
        ...
        try{
            ...
            "http://localhost:5000/api/sub-subcategories/assign/master-image",{
