...
const OneCategoryMaster = ({ onClose, categoryId }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const catRes = await axios.get(
          `https://mybackend-l7om.onrender.com/api/categories/${categoryId}`,
        ...
        const masterRes = await axios.get(
          'http://localhost:5000/api/items/getImageByCategory',
          {
            headers: {
        ...
        setMasterImages(allImages[categoryId] || []);
      } catch (err) {
  ...
  const handleRegenerateMasterImage = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/items/category/assign-masterImage/?page=${page}`,
        {
          params: { categoryId },
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const handleSetAsMaster = async (image) => {
    try {
      const res = await axios.put(
        `http://localhost:5000/api/category/assign/master-image`,
        {
          id:categoryId,
          image: image
        },
      ...
      alert("Failed to set master image.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative w-11/12 max-w-4xl p-6 bg-white rounded-lg shadow-lg">
        ...
        <p className="mb-4 text-center text-gray-500">Category ID: {categoryId}</p>
...
export default OneCategoryMaster;
