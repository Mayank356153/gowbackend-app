          ...
          <div className="p-4 mt-5 overflow-auto border-t-4 rounded-lg border-cyan-500">
            ...
            <div className="flex justify-center mb-4">
              <select
                ...
                className="w-full max-w-xs px-4 py-2 bg-white border rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-400"
              >
            ...
            {searchQuery.trim() !== "" && filteredItems.length > 0 && (
              <ul className="p-2 mb-4 overflow-y-auto bg-white border rounded shadow max-h-60">
                {filteredItems.map((item) => (
                  <li
                    key={item._id}
                    className="flex items-center justify-between px-3 py-2 border-b cursor-pointer last:border-none hover:bg-gray-50"
        ...
        <div className="p-4">
          <div className="grid grid-cols-3 gap-4">
            {printList.map((item) => (
              <div
                key={item._id}
                className="p-3 text-center border break-inside-avoid"
                style={{ pageBreakInside: "avoid" }}
              >
                <div className="mb-2 text-lg font-bold">
                  {item.itemName}
                </div>
                <div className="mb-1 text-sm">Code: {item.itemCode}</div>
                <div className="mb-1 text-sm">Qty: {item.quantity}</div>
