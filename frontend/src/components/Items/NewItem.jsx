...
useEffect(() => {
    axios
      .get(`${link}/api/items/available-images`, headers.current)
      .then((r) => {
        console.log("Preloaded Images:", r.data);
  ...
  const saveItem = async () => {
    ...
    const url = id ? `${link}/api/items/${id}` : `${link}/api/items/create/sql`;
  ...
  const onSearch=async(query)=>{
    try {
        if(query === "") return;
      const { data } = await axios.get(`https://pos.inspiredgrow.in/vps/api/new-items/search?q=${query}`,{
         headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`
      },
      });
...
const category=(s,name)=>{
    console.log(name)
    const v= options[name].find((cat) => cat.label?.toLowerCase() === s?.toLowerCase())?.value || null;
     console.log(v);
    return v;
}
  ...
  const setResult=(item)=>{
    ...
    setFormData({
        ...
        category:category(item.CategoryName,"category") || null,
        brand:category(item.BrandName,"brand") || null,
        subCategory:category(item.SubCategoryName,"subCategory") || null,
        subSubCategory:category(item.SubSubCategoryName,"subsubCategory") || null,
        tax:category(item.TaxName,"tax") || null,
        unit:category(item.UnitName,"unit") || null,
        itemName:item.ItemName,
        alertQuantity: item.AlertQuantity || 0,
        barcodes: [item.Barcodes] || [],
        discount:Number(item.Discount) || 0,
        discountType: item.DiscountType || "Percentage",
        discountPolicy:item.DiscountPolicy || "None",
        expiryDate: item.ExpiryDate || null,
        freeQuantity: Number(item.FreeQuantity) || 0,
        hsn:item.HSN,
        description:item.ItemDescription,
        itemGroup:item.ItemGroup || "",
        itemImages:[item.ImageCode] || [],
        mrp:Number(item.MRP) || 0,
        openingStock:Number(item.OpeningStock) || 0,
        priceWithoutTax: Number(item.PriceWithoutTax) || 0,
        purchasePrice:Number(item.PurchasePrice) || 0,
        requiredQuantity:item.RequiredQuantity || 0,
        sku:item.SKU || "",
        salesPrice:Number(item.SalesPrice) || 0,
        sellerPoints:Number(item.SellerPoints) || 0,
        taxType:item.TaxType==="Inclusive"?"inclusive":"exclusive",
        isOnline:item.Visibility==="online"?true:false,
    })
    setValue("");
  }
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      {/* Search Bar */}
      <div
        className={`flex items-center w-full rounded-xl bg-white shadow-sm border border-gray-200 focus-within:ring-2 focus-within:ring-blue-500`}
      >
        {/* Search Icon */}
        <svg
        ...
        >
          <path
            ...
            strokeWidth={1.5}
            d="M21 21l-4.3-4.3m0 0A7 7 0 104.3 4.3a7 7 0 0012.4 12.4z"
          />
        </svg>
        ...
        <button
          ...
          className="px-3 py-2 m-1 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700"
        >
          Search
        </button>
      </div>
      ...
      {value && searchResults.length > 0 && (
        <div className="absolute z-10 mt-3 overflow-y-auto bg-white border border-gray-200 rounded-lg shadow max-h-40">
          {searchResults.map((item) => (
            <div onClick={() => setResult(item)}
              key={item.ItemID}
              className="px-4 py-2 border-b last:border-b-0 hover:cursor-pointer hover:bg-gray-100"
            >
            ...
            <div className="flex flex-col gap-5 p-4 mt-4 bg-white border-t-2 rounded-lg border-t-blue-600">
                    ...
                    <table className="min-w-full border border-collapse table-fixed">
                      <thead>
                        <tr className="text-sm bg-gray-300">
                          <th className="w-1/6 p-2 border">Variant</th>
                          <th className="w-1/12 p-2 border">SKU</th>
                          <th className="w-1/12 p-2 border">HSN</th>
                          <th className="w-1/6 p-2 border">Barcodes</th>
                          <th className="w-1/12 p-2 border">Price</th>
                          <th className="w-1/12 p-2 border">Purchase</th>
                          <th className="w-1/12 p-2 border">Profit %</th>
                          <th className="w-1/12 p-2 border">Sales</th>
                          <th className="w-1/12 p-2 border">MRP</th>
                          <th className="w-1/12 p-2 border">Opening</th>
                          <th className="w-1/12 p-2 border">Policy</th>
                          <th className="w-1/12 p-2 border">Req</th>
                          <th className="w-1/12 p-2 border">Free</th>
                          <th className="w-1/12 p-2 border">Disc %</th>
                          <th className="w-1/12 p-2 border">Action</th>
                        </tr>
                      </thead>
                      <tbody>
